...
abstract base class DartEdgeHttpClientBase {
  ...
  Future<DartEdgeClientResponseObject<TResponse>> invoke<
    TResponse,
    TParams,
    TQuery,
    THeaders,
    TBody
  >(DartEdgeClientInvocation<TResponse, TParams, TQuery, THeaders, TBody> invocation) async {
  ...
  Future<DartEdgeClientStreamedResponseObject> invokeStream<TParams, TQuery, THeaders, TBody>(
    DartEdgeClientInvocation<Object?, TParams, TQuery, THeaders, TBody> invocation,
  ) async {
  ...
  Future<DartEdgeClientWebSocket> connectWebSocket<TParams, TQuery, THeaders>(
    ...
    if (transport == null) {
      throw StateError('No DartEdgeClientWebSocketTransport configured for this client.');
    }
  ...
  Future<DartEdgeClientWebTransportSession> connectWebTransport<TParams, TQuery, THeaders>(
    DartEdgeClientWebTransportInvocation<TParams, TQuery, THeaders> invocation,
  ) {
    final transport = webTransportTransport;
    if (transport == null) {
      throw StateError('No DartEdgeClientWebTransportTransport configured for this client.');
    }
  ...
  Future<DartEdgeClientRequest> _buildClientRequest<TResponse, TParams, TQuery, THeaders, TBody>(
    DartEdgeClientInvocation<TResponse, TParams, TQuery, THeaders, TBody> invocation,
  ) async {
    final abortTrigger = _combinedAbortTrigger(
      abortTrigger: invocation.abortTrigger,
      timeout: invocation.timeout,
  ...
  Future<_EncodedClientRequestBody?> _encodeRequestBody<TBody>({
    required DartEdgeClientRequestBody<TBody>? body,
    required void Function(DartEdgeMultipartUploadProgress progress)? onMultipartUploadProgress,
  }) async {
...
_EncodedMultipartFormData _encodeMultipartFormData(MultipartFormData form) {
  final boundary = 'dart-edge-boundary-${DateTime.now().microsecondsSinceEpoch}';
  return (
    boundary: boundary,
...
Stream<List<int>> _trackMultipartUploadProgress(
  ...
  await for (final chunk in stream) {
    ...
    onProgress(DartEdgeMultipartUploadProgress(bytesSent: bytesSent, totalBytes: totalBytes));
    yield chunk;
  }
}
...
final class DartEdgeClientInvocation<TResponse, TParams, TQuery, THeaders, TBody> {
  ...
  final void Function(DartEdgeMultipartUploadProgress progress)? onMultipartUploadProgress;
}
...
final class DartEdgeClientRequestValue<T> {
  const DartEdgeClientRequestValue({this.schemaId, required this.value, this.encoder});
...
final class DartEdgeClientDownloadProgress {
  const DartEdgeClientDownloadProgress({required this.bytesReceived, required this.totalBytes});
