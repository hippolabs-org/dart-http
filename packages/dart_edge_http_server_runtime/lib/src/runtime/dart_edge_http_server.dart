...
class DartEdge<TServices> extends Router<TServices> {
  ...
  Future<void> _handleWebSocketHandshake(
    ...
    final accepted = DartEdgeNative.acceptWebSocket(requestId, headers: ctx.res.headers);
    if (!accepted) {
  ...
  Future<void> _handleWebTransportHandshake(
    ...
    final accepted = DartEdgeNative.acceptWebTransport(requestId, headers: ctx.res.headers);
    if (!accepted) {
      ...
      writeLease: (lease) => stream.enqueueSend(() async {
        try {
          final operationId = switch (lease) {
            NativeBinaryPayloadLease() => DartEdgeNative.webTransportStreamWriteNative(
              info.streamId,
              bodyPtr: lease.bytesPtr,
              bodyLength: lease.length,
            ),
            _ => DartEdgeNative.webTransportStreamWrite(info.streamId, lease.bytesView),
          };
  ...
  TServices _createServices() {
    ...
    throw StateError('No services factory configured. Pass services: () => ... to DartEdge.');
  }
