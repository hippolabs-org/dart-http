#ifndef DART_HTTP_SERVER_RUNTIME_H_
#define DART_HTTP_SERVER_RUNTIME_H_

#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>

#include <dart_http_core_ffi.h>
#include <dart_http_server_core.h>

typedef void (*dart_http_server_runtime_transport_event_callback_t)(
    int32_t event_kind,
    int64_t event_id);
...
int32_t dart_http_server_runtime_native_abi_version(void);

int64_t dart_http_server_runtime_start_server(
    const char* host,
    int64_t port,
...
void dart_http_server_runtime_stop_server(void);

void dart_http_server_runtime_stop_server_by_id(int64_t server_id);

NativeTransportRequest* dart_http_server_runtime_take_request(int64_t request_id);
...
bool dart_http_server_runtime_accept_web_transport(
    int64_t request_id,
    intptr_t header_count,
    const NativePair* headers);

bool dart_http_server_runtime_start_sse_response(
    int64_t request_id,
    int32_t status,
    intptr_t header_count,
    const NativePair* headers);

bool dart_http_server_runtime_send_sse_chunk(
    int64_t request_id,
    const char* chunk);

bool dart_http_server_runtime_finish_sse_response(int64_t request_id);

bool dart_http_server_runtime_start_binary_stream_response(
    int64_t request_id,
    int32_t status,
...
bool dart_http_server_runtime_start_native_binary_stream_response(
    int64_t request_id,
    int32_t status,
    const char* content_type,
    int64_t content_length,
...
void dart_http_server_runtime_free_web_socket_connection(
    NativeWebSocketConnection* value);

NativeWebSocketMessage* dart_http_server_runtime_take_web_socket_message(
    int64_t session_id);
...
void dart_http_server_runtime_free_web_transport_connection(
    NativeWebTransportConnection* value);

NativeWebTransportDatagram* dart_http_server_runtime_take_web_transport_datagram(
    int64_t session_id);

void dart_http_server_runtime_free_web_transport_datagram(
    NativeWebTransportDatagram* value);

NativeWebTransportStream* dart_http_server_runtime_take_web_transport_stream(
    int64_t session_id);

void dart_http_server_runtime_free_web_transport_stream(
    NativeWebTransportStream* value);

NativeWebTransportStreamInfo* dart_http_server_runtime_take_web_transport_stream_info(
    int64_t stream_id);
...
void dart_http_server_runtime_free_web_transport_stream_chunk(
    NativeWebTransportStreamChunk* value);

NativeWebTransportStreamTerminal* dart_http_server_runtime_take_web_transport_stream_terminal(
    int64_t stream_id);

void dart_http_server_runtime_free_web_transport_stream_terminal(
    NativeWebTransportStreamTerminal* value);

NativeWebTransportOperation* dart_http_server_runtime_take_web_transport_operation(
    int64_t operation_id);

void dart_http_server_runtime_free_web_transport_operation(
    NativeWebTransportOperation* value);

int64_t dart_http_server_runtime_web_transport_open_unidirectional_stream(
    int64_t session_id);

int64_t dart_http_server_runtime_web_transport_open_bidirectional_stream(
    int64_t session_id);

int64_t dart_http_server_runtime_web_transport_stream_write(
    int64_t stream_id,
    NativeBytes body);

int64_t dart_http_server_runtime_web_transport_stream_finish(
    int64_t stream_id);

int64_t dart_http_server_runtime_web_transport_stream_reset(
    int64_t stream_id,
    uint32_t error_code);

int64_t dart_http_server_runtime_web_transport_stream_stop(
    int64_t stream_id,
    uint32_t error_code);

bool dart_http_server_runtime_web_transport_send_datagram(
    int64_t session_id,
    NativeBytes body);

bool dart_http_server_runtime_web_transport_send_stream(
    int64_t session_id,
    NativeBytes body);

bool dart_http_server_runtime_web_transport_close(
    int64_t session_id,
    int32_t code,
    const char* reason);
...
bool dart_http_server_runtime_web_socket_close(
    int64_t session_id,
    int32_t code,
    const char* reason);

char* dart_http_server_runtime_take_last_error(void);

void dart_http_server_runtime_free_string(char* value);

bool dart_http_server_runtime_send_response(
    int64_t request_id,
    int32_t status,
    const char* content_type,
    NativeBytes body,
...
#endif  // DART_HTTP_SERVER_RUNTIME_H_
